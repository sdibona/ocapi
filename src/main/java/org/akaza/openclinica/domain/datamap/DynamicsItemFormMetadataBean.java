...
public class DynamicsItemFormMetadataBean extends AbstractMutableDomainObject {
    ...
    public DynamicsItemFormMetadataBean(ItemFormMetadata metadataBean, EventCrf eventCRFBean) {
        setItemId(metadataBean.getItem().getItemId());
        setItemFormMetadataId(metadataBean.getItemFormMetadataId());
        setCrfVersionId(metadataBean.getCrfVersionId());
        setEventCrfId(eventCRFBean.getEventCrfId());
        setPassedDde(0);
    }
    
    public DynamicsItemFormMetadataBean(ItemFormMetadata metadataBean, EventCrf eventCRFBean, ItemData itemData) {
        setItemId(metadataBean.getItem().getItemId());
        setItemFormMetadataId(metadataBean.getItemFormMetadataId());
        setCrfVersionId(metadataBean.getCrfVersionId());
        setEventCrfId(eventCRFBean.getEventCrfId());
        setItemDataId(itemData.getItemDataId());
        setPassedDde(0);
    }
