...
import org.akaza.openclinica.domain.dao.*;
...
import org.akaza.openclinica.util.Utils;
...
public class XformMetaDataService {
    ...
    public Errors runService(CrfVersion version, XformContents xformContents, Study currentStudy, UserAccount ub, Html html, String submittedCrfName,
        ...
        try {
            createCRFMetaData(version, xformContents, currentStudy, ub, html, submittedCrfName, submittedCrfVersionName, submittedCrfVersionDescription,
    ...
    @Transactional
    public CrfVersion createCRFMetaData(CrfVersion version, XformContents xformContents, Study currentStudy, UserAccount ub, Html html,
                                        ...
                                        String submittedXformText, List<MultipartFile> formItems, Errors errors) throws Exception {

        // Retrieve CrfBean. Create one if it doesn't exist yet.
        CrfBean crf = null;
        if (version.getCrf().getCrfId() > 0) {
            crf = (CrfBean) crfDao.findById(version.getCrf().getCrfId());
            crf.setUpdateId(ub.getUserId());
            ...
            crf.setUserAccount(userDao.findById(ub.getUserId()));
            crf.setStatus(org.akaza.openclinica.domain.enumsupport.Status.AVAILABLE);
            crf.setStudy(studyDao.findByStudyId(currentStudy.getStudyId()));
...
//            crf.setCrfId(crfId);
        ...
        crfVersion.setUserAccount(userDao.findById(ub.getUserId()));
        crfVersion.setStatus(org.akaza.openclinica.domain.enumsupport.Status.AVAILABLE);
        ...
        crfVersion.setXformName(xformContents.getInstanceName());
        //Integer crfVersionId = (Integer) crfVersionDao.save(crfVersion);
        //crfVersion.setCrfVersionId(crfVersionId);
        crfVersion = crfVersionDao.save(crfVersion);

        // Create Section
        Section section = new Section();
        section.setCrfVersion(crfVersion);
        section.setStatus(org.akaza.openclinica.domain.enumsupport.Status.AVAILABLE);
        section.setLabel("");
        section.setTitle("");
        section.setSubtitle("");
        section.setPageNumberLabel("");
        section.setOrdinal(1);
        section.setUserAccount(userDao.findById(ub.getUserId())); // not null
        ...
        createGroups(xformContents, html, submittedXformText, crf, crfVersion, section, ub, errors);
    ...
    private void saveMedia(List<MultipartFile> items, CrfBean crf, CrfVersion version) {
    ...
    private void createGroups(XformContents xformContents, Html html, String submittedXformText, CrfBean crf, CrfVersion version, Section section,
            UserAccount ub, Errors errors) throws Exception {
        ...
        for (Group htmlGroup : htmlGroups) {
            XformGroup xformGroup = xformContents.findGroupByRef(htmlGroup.getRef());
            ...
            if (itemGroup == null) {
                ...
                itemGroup.setStatus(org.akaza.openclinica.domain.enumsupport.Status.AVAILABLE);
                ...
                itemGroup = itemGroupDao.save(itemGroup);
            }
            ...
            for (UserControl widget : widgets) {
                ...
                XformItem xformItem = xformContents.findItemByGroupAndRef(xformGroup, widget.getRef());
    ...
    private Item createItem(Html html, UserControl widget, XformGroup xformGroup, XformItem xformItem, CrfBean crf, UserAccount ub,
        ...
        if (item != null) {
            ...
            item.setStatus(org.akaza.openclinica.domain.enumsupport.Status.AVAILABLE);
